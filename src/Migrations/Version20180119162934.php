 ...
 */
class Version20180119162934 extends AbstractMigration
{
    public function up(Schema $schema)
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__category AS SELECT id, name, image, updated_at, description FROM category');
        $this->addSql('DROP TABLE category');
        $this->addSql('CREATE TABLE category (id INTEGER NOT NULL, name VARCHAR(100) NOT NULL COLLATE BINARY, image VARCHAR(255) DEFAULT NULL COLLATE BINARY, updated_at DATETIME DEFAULT NULL, description CLOB DEFAULT NULL COLLATE BINARY, slug VARCHAR(128) DEFAULT NULL, PRIMARY KEY(id))');
        $this->addSql('INSERT INTO category (id, name, image, updated_at, description) SELECT id, name, image, updated_at, description FROM __temp__category');
    ...
    public function down(Schema $schema)
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__category AS SELECT id, name, description, image, updated_at FROM category');
        $this->addSql('DROP TABLE category');
        $this->addSql('CREATE TABLE category (id INTEGER NOT NULL, name VARCHAR(100) NOT NULL, description CLOB DEFAULT NULL, image VARCHAR(255) DEFAULT NULL, updated_at DATETIME DEFAULT NULL, PRIMARY KEY(id))');
        $this->addSql('INSERT INTO category (id, name, description, image, updated_at) SELECT id, name, description, image, updated_at FROM __temp__category');
        $this->addSql('DROP TABLE __temp__category');
        $this->addSql('DROP INDEX IDX_C5E24E1812469DE2');
        $this->addSql('DROP INDEX IDX_C5E24E187294869C');
        $this->addSql('CREATE TEMPORARY TABLE __temp__category_article AS SELECT category_id, article_id FROM category_article');
